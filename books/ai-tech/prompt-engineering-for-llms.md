...
information. Y ou can access this page at https://oreil.ly/PromptEngForLLMs.
...
adding) biased commentary. Y ou’ll use LLMs to assist in your communications by
...
to Google for, but some would push even a senior developer to Stack Overflow. Y et, if
...
model is to predict the probability of the next word. Y ou’ve seen this functionality
before, haven’t you? It’s the bar of completion words that appears above the keypad
when you’re typing out a text message on your iPhone (see Figure 1-2). Y ou might
...
1. Tokens from the source language are sent to the encoder one at a time and1.
converted to an embedding vector, and they update the internal state of the
encoder.
2. The internal state is packaged up as the thought vector and sent to the decoder.2.
...
3. A special “start” token is sent to the decoder, indicating that this is the start of the3.
output tokens.
4. Conditioned upon the value of the thought vector, the decoder state is updated4.
and an output token from the target language is emitted.
5. The output token is provided as the next input into the decoder. At this point, the5.
process recurrently loops back and forth from step 4 to step 5.
6. Finally, the decoder emits a special “end” token, indicating that the decoding6.
...
Y ou Need”, which introduced the transformer architecture shown in Figure 1-4. The
...
wrapping the conversation thread in a special ChatML markdown. (Y ou’ll learn more
...
Y ou may choose to drop the earliest exchanges or less relevant content from previous
exchanges, and you may even employ summarization to compress the content.
Y et another aspect of prompt engineering at this level of sophistication involves
...
and autonomy is still an important step toward exciting future possibilities. Y ou’ll
...
In this chapter, we’ll approach this problem onion style. Y ou’ll first see LLMs from
the very outside as trained mimics of text in “What Are LLMs?” on page 16. Y ou’ll
...
that split.
Y ou’ll also find out how the token sequences are generated bit by bit in “One Token
...
across at the podcasting site Acast. Y ou might start writing the following: In their
...
any rate, he didn’t talk about the Hundred Y ears’ War on the podcast The  Rest Is
History.
...
5 Y ou can check this by making a second query to the LLM. See Chapter 7.
...
Y ou can make truth bias work for you—if you want the model to assess a hypothetical
...
Y ou can have lots of fun with purposely garbled sentences just at the edge of what
...
ization. (Y ou don’t need an LLM to capitalize text after all!) So the wise prompt
engineer will try to avoid burdening the models overmuch by having the LLM
translate between capitalizations all the time.
Counting Tokens
Y ou can’t mix and match tokenizers and models. Every model uses a fixed tokenizer,
...
than complex token boundary analysis. Y ou’ll most often use the tokenizer just for
counting.
...
Most tokenizers are optimized for English 6 and will be less efficient for other lan‐
...
Y ou know how when you write text on your phone, you can get three-word sugges‐
...
the real gift Ilúvatar’s Y ounger Children should treasure above all others.
...
0
Y ou want the most likely token. No alternatives. This is the recommended setting
...
0.5–0.7
Y ou want a greater impact of chance on the solution, and you are fine with get‐
...
10 or more.
1
Y ou want the token distribution to mirror the statistical training set distribution.
...
> 1
Y ou want a text that’s “more random” than the training set. This means the model
...
Y ou peel it back and see….it’s not one brain at all. It’s thousands of minibrains. All
...
1. Each minibrain has some things it wants to know, so it submits a couple of1.
...
talking?”
2. Each minibrain has some things it can share, so it submits a couple of items, in2.
...
right now is Susan. ”
3. Now, every question is matched up with its best-fitting answer. “Who is talking?”3.
matches up very well with “The person talking right now is Susan. ”
4. The best-fitting answer to each question is revealed to the minibrain that asked4.
...
vectors of numbers 13 and that is unique to every LLM, since it’s something the
...
chat. Y ou’ll learn about the implications of RLHF for prompt engineering and LLM
...
chat.
Prompts and corresponding human-
generated ideal completions
~13,000 documents
...
for dialogue and for the behavior of the assistant. Y ou are free to write whatever
...
expected behavior. For instance, it says, “Y ou are a software assistant, and you provide
...
“Y ou are Rick Sanchez from Rick and Morty. Y ou are quite profane, but you provide
...
message. Y ou might be tempted to add your user’s request to the
...
answer, not an editorial commentary on the answer. Y ou’ll feel this most sharply
...
Y ou can even specify the stop parameter to be ```, and then, there will be nothing
...
want it to be filtered through a nice assistant. Y ou want the raw humanity, which,
...
Take a look at Table 3-6. One of the playwrights is you—the prompt engineer. Y ou
...
placing the request in backticks). Y ou’ll also need to update the application to actually
...
takes, the problem of building an LLM application remains much the same. Y ou, the
...
simultaneously:
1. The prompt must closely resemble content from the training set.1.
2. The prompt must include all the information relevant to addressing the user’s2.
problem.
3. The prompt must lead the model to generate a completion that addresses the3.
problem.
4. The completion must have a reasonable end point so that generation comes to a4.
natural stop.
...
principle. Y ou remember that story, right? A naive girl dressed in fashionable red
...
specifying financial information about a company?"  Y ou
...
Y ou must therefore carefully consider how to set up the prompt so that it points to a
solution. When working with completion models, this can be surprisingly tricky. Y ou
...
But this doesn’t mean that you, as the prompt engineer, are off the hook! Y ou’re
fully responsible for including all the relevant information for addressing the user’s
problem (criterion 2). Y ou must craft the text within the chat so that it resembles
...
1. What if you leave off ## Solution 2 or even the question that precedes it? Does1.
...
2. Problem 1 serves as an example. If you change the solution 1 text, does it modify2.
...
3. Try keeping the same country but replacing the negative context with increas‐3.
ingly positive remarks. Does the model still recommend against travel to North
Korea? Why might this be?
4. If you omit the stop word, then does the model confabulate a third problem? If4.
...
5. Are there any reasons that using a homework problem might be problematic?5.
...
Y ou’ll have to decide how big your model should be. Typically, the larger the model
...
user’s domain.
Y ou can go further by giving the model functions that actually create a change in
...
be used first.
Prompt assembly
In the last step, all of this snippet fodder gets assembled into the final prompt. Y ou
...
It’s at this point where accounting comes heavily into play. Y ou must make sure that
...
are accomplishing their tasks efficiently (Y ay!), but it could also be that users are
...
into all of the topics introduced in this chapter. Y ou’ll learn more about where to pull
...
want to find lots and lots of potential content. Y ou can whittle down what you find
...
Y ou want your few-shot examples to be of the same type as the question you’re
...
Y ou’ve gathered lots of context about the user: demographics, Amazon reviews they
left, books they recently bought, their biography, and their favorite flavor of ice
cream. Y ou knew in advance that you’ d have gathered this context for anyone, so you
...
quite possible. Y ou can and should try to provide a good range of examples so you
...
influence the model.
Y our prompt examples are not randomly ordered unless you consciously shuffle
...
user is inactive or fire-
and-forget action by
user
Email summarization
assistant
...
gather context, your aim should be to gather more than you can use. Y ou may need
...
it. Actually getting it might be difficult. Y ou could find current bestsellers with a
...
Y ou can often easily sort the context you can gather according to several dimensions,
...
1. Anything the application has directly at its fingertips, like anything to do with the1.
current state of the application (e.g., what’s currently written on the screen) or the
system (e.g., the current time and date)
2. What the application has saved somewhere (e.g., the user’s profile info)2.
3. Information the application could record for itself, even if it doesn’t yet (e.g.,3.
...
4. Information the application could obtain by using public APIs (e.g., the current4.
weather)
5. Information the application could obtain by asking the user directly or accessing5.
...
1. Things that are always the same for the same user (e.g., profile information)1.
2. Things that change slowly over time (e.g., purchase histories)2.
3. More ephemeral things (e.g., time, states of the user’s interaction with the app)3.
...
novel The  Beach. Y our app has been to Wikipedia and copied over the summary of
The  Beach:
...
1. Split up the documents into smaller snippets.1.
2. Convert all of the snippets into embedding vectors, using the process described2.
above.
3. Insert the snippets and their corresponding vectors into a vector datastore of3.
your choice.
...
1. Make sure that the number of tokens is less than the maximum number of tokens1.
allowed for your embedding model. (As of 2024, the OpenAI embedding models
have a window of 8,191 tokens.)
2. Ideally, make sure that the text chunk is large enough to hold one and only one2.
...
3. Make sure the snippet is an appropriate size for placement in the prompt.3.
...
window boundary. However, you are in control of this decision. Y ou might want to
...
tokens in the query don’t match the tokens in the document. Y ou can fix problems
...
Y ou can do this by modifying how the relevance score is weighted based upon
...
solution. Y ou can reduce its impact by placing key, high-quality prompt elements
...
Natural interaction
It’s easy for people to think in terms of conversations. Y ou can pose a question
...
in two ways:
1. For chain-of-thought prompting or managing overly verbose models, you can1.
...
2. Y ou can easily signal when the model’s response should end by adding a section2.
...
in such text fields, you don’t need to escape anything, which is nice. Y ou’ll notice that
...
Modularity
Y ou want your snippets to be strings that can be inserted into or removed from
...
Inertness
Y ou’ d like to compute the token length of a snippet only once, so the tokenization
...
context from the book in your prompt. Y ou remember how to retrieve relevant book
passages from Chapter 5, and let’s say you identify two key moments.
Y ou can snippetize these passages in different ways, as shown in Figure 6-5. Ideally,
...
ments:
1. Y ou could use what we call elastic prompt elements, which are prompt elements1.
...
2. Alternatively, you could create multiple prompt elements from the retrieved2.
...
even more context. Y ou’ll have to remember to only actually include one of these,
...
dependency. Y ou’ll need to keep these three dimensions in mind while you’re con‐
...
ing. Y ou also need to test and refine these importance parameters with the methods
...
Y ou have two main constraints:
...
Y ou’ve also learned how to convert your information into prompt elements—snippets
...
But there’s a second, at least equally important consideration that applies here. Y ou
...
generation will stop (on the server side, if it’s on a server) and end its answer. Y ou
...
are some of the most common ways. Y ou might think that \ndef
...
logprobs for “Y es” and “No” are ‒0.405 and ‒1.099, respectively, then the model is
about 66% sure it will be “Y es” and 33% sure it will be “No. ”
...
Y ou can do many cool things with logprobs. Let’s talk about how to use them to eval‐
...
1. Only allow your application to show corrections if it is confident.1.
2. Include warnings when the model struggles more than usual.2.
3. Incorporate more context or retry when the model struggles.3.
4. Switch to a more intelligent (and expensive) LLM for better results.4.
5. Only interrupt the user with assistance if the certainty that it is necessary is high.5.
...
Y ou can let the model make all kinds of decisions through classification, but in many
...
to rewrite them. Y ou can pretty easily ask the model, “Is this a professionally written
email? Please use the format 1. Y es / No. 2. Explanation. ” But even if the model is
...
if the logprob for No is at least 0.3 higher than the one for Y es.
Generally, to calibrate the LLM’s decision process, you shift the logprobs by a con‐
stant (where each a tok corresponds to one of the tokens in question). For example,
you can make the email classification less strict by adding a constant like a yes = 0.3
to the logprob of “Y es” before comparing it with the logprob for No. Y ou can find
...
regression.
If you found constants a tok that you like, you don’t actually have to
mess with the logprobs anymore—many model providers offer in
their API the possibility of a logit bias, where you send the a tok to
...
prompt. Y ou can run this to better understand the text you send to the model, even if
...
into your code too firmly. Y ou may want to revise, evaluate, and
...
When you’re deciding on a model, the first step is usually picking a provider. Y ou’ll
...
during prototyping. Y ou’ll be glad if your prompt engineering and
postprocessing is already optimized for the better models you now
can afford.
Y ou probably won’t ever want to build and train your own model from scratch,
...
can you gather these examples? Y ou can create some yourself, hire contractors, or
...
destinations. Y our app telemetry might show that suggesting Monaco usually gets a
...
will just bake them into its parameters. Y ou also don’t need few-shot prompting
...
1. Try to make your prompt look like the beginning of one of the documents you1.
fine-tuned for.
2. Be very sure not to have it look like one of the original documents instead.2.
...
zeitgeist of the world. Y ou can learn anything you want from a broad range of topics,
...
that the model has no access to. Y ou also work in the present, not the past, and
...
1. Who should speak? The OpenAI API, rather than the model, inserts <|im_start|1.
...
2. Should a tool be called?  The next tokens, to=functions., are generated by the2.
...
3. Which tool should be called?  The next tokens the model generates represent the3.
name of the function: in this case, set_room_temp\n.
4. Which argument should be specified?  The next text generated from the model4.
...
5. What value will the argument have?  The model next predicts the value that5.
...
6. Are we done?  Once all the arguments have been specified, the model predicts6.
...
• Y ou know that somewhere in the system message, there has to be text of the•
...
1. Whatever is easier for a human to understand is also easier for an LLM to1.
understand.
...
2. The best results are derived by patterning prompts after training data (a.k.a. the2.
...
fine with all of the JSON schema types: string, number, integer, and boolean. Y ou
Tool Usage | 179
...
1. When the desired value is known in the application, remove the arguments from1.
...
tion.
2. Instruct the model to ask if it’s unsure about an argument—and then pray it does,2.
...
answer this question. Y ou would probably look up both of these magazines, find the
...
tions—each of which tackles the problem in isolation. Y ou could just have them make
...
Current exchange: the current user
request
...
• What artifacts should you present? Y our options are as follows:•
...
important.
• How should artifacts be presented? Y our options are as follows:•
...
then certainly, you wouldn’t include the full text in the prompt. Y ou wouldn’t be
...
Y our UI should indicate when the agent is using tools, for instance, with a pill
...
Conclusion
Y ou’ve come a long way in this chapter. Y ou’ve learned that agency is the ability
of an entity to accomplish tasks in a self-directed manner. Y ou’ve also learned that
...
implementation details, this chapter keeps the discussion high level. Y ou should be
...
1. Generate a list of popular Shopify storefronts and retrieve their website HTMLs.1.
2. For each storefront, extract details—product offering, branding, style, values, etc.2.
3. Review each storefront and come up with a plug-in that would benefit its busi‐3.
ness.
4. Generate marketing emails advertising the plug-in concept to each storefront4.
owner.
5. Send the emails.5.
This sounds like a pretty nutty idea, right? Y ou’re basically blasting out emails for
...
a sock store. It was a web page called Sock-cess Stories (see Figure 9-3). Y ou have to
...
tem message: “Y ou are a helpful assistant. Y ou can do anything— you just have to
...
system message covering all of the problematic aspects just mentioned. Y ou might
...
1. Define  goal. Identify the purpose of the workflow. What is the desired output or1.
desired change that the workflow will accomplish?
2. Specify tasks. Break the workflow down into a set of tasks that, when executed in2.
...
3. Implement tasks. Build the tasks as specified. Make sure input and output are3.
...
4. Implement workflow . Connect the tasks into a complete workflow. If necessary,4.
adjust tasks to ensure they function correctly in the context of the full workflow.
5. Optimize workflow . Optimize tasks to improve quality, performance, and cost.5.
...
1. Generate a list of popular Shopify storefronts and retrieve their website HTMLs.1.
2. For each storefront, extract details—product offering, branding, style, values, etc.2.
3. Review each storefront and come up with a plug-in that would benefit their3.
business.
4. Generate marketing emails advertising the plug-in concept to each storefront4.
owner.
5. Send the emails.5.
...
upbeat, it should compliment the store owner, etc. Y ou might also provide more
...
analysis could even ask an LLM to review the output. (Y ou’ll hear this referred to as
...
Y ou may also want to incorporate human interaction in tasks. If any task requires
...
of ways. Y ou can think of the workflow as being a state machine in which each task
...
be quite useful. Y ou can get around this problem by passing the extraction details
...
1. What do they sell?1.
2. What is the overall tone of the website? Fun? Serious? Relaxing?2.
...
3. What values do they hold most dear? Sustainability? Social causes?3.
4. What themes are present on the website? Travel? Productivity? Exercise?4.
5. Is there anything praiseworthy on the website? (We’re getting ready to stroke5.
their ego in the email!)
6. Is there anything else that seems noteworthy?6.
...
to implement. Y ou should probably add a subprocess to plan out the implementation
...
evaluates task output and then prompts the model for improvements. Y ou can also
...
tasks that will properly handle it. Y ou can implement this at the workflow level by
...
Y ou can go further down this path. In addition to making the workflow a conversa‐
...
Y ou could make the workflow agent act as an orchestrator, sending requests for
...
again. ” “Y ou bet, thank you too!” and so on. Finally, modify their system messages
...
• The model you use•
• Y our individual interactions with the model (i.e., your prompts)•
...
declaring “This is what I’m testing now!” Y ou’re not free in that choice; particular
...
want to capture as large a part of the app as possible. Y ou can test each unit
...
could later evolve into one). Y ou’ll have no automated way of knowing whether
...
1. Where do you get the example problems?1.
2. How do you assess your app’s solutions to those problems?2.
Y ou can upgrade from playground tinkering to an example suite once you have your
...
• Y ou give up on evaluating the whole loop, and instead, you evaluate individual•
...
• Y ou can use the model to mock out the user’s side of the conversation. In•
...
• Y ou’ll have to make them up entirely.•
...
1. Take an open source repository, and from it, take one code file, and from that file,1.
take one function.
2. Remove the body of that function, imagining that the user was just writing2.
...
would go.
3. Ask GitHub Copilot what to type next.3.
...
Y ou can run your app on them and get a candidate solution for each of them, but now
what?
...
However, very often, the output of an LLM is more or less free-form text. Y ou can use
...
Y ou’re looking at a situation where the user says, “I’m chilly” (see Figure 10-3 ).
Y ou’ve already determined a gold standard solution for this: the system could set the
...
sion A is considered right more often than version B. ”). Y ou may
...
Y ou should break apart any Goldilocks questions that ask whether
a completion was “just right. ” Those questions really capture two
aspects: it was enough, and it wasn’t too much. Y ou typically get
...
no substantial regression by doing that. Y ou could let a human annotate some cases
...
also to avoid putting some real stinkers in front of your users. Y ou define in advance
which metrics to look for that you want to optimize; often, they are proxies for user
satisfaction (e.g., average rating, acceptance rate). Y ou may also define some guardrail
...
in mode A, and the rest gets the app running in mode B. Y ou let the experiment run
...
3 Y ou can’t just say, “Let’s put the users who already updated in group B (‘the new thing’) and the others in
...
tion. Y ou have only a finite number of users, and getting a signal
...
prompting idea to all (most 3) users before you can even start testing it. That rollout
...
follows:
1. Direct feedback: what does the user say to the suggestion?1.
2. Functional correctness: does the suggestion work?2.
3. User acceptance: does the user follow the suggestion?3.
4. Achieved impact: how much does the user benefit?4.
5. Incidental metrics: what are the measurements “around” the suggestion?5.
...
for smaller subtasks in a greater routine. Y ou wanted to open a program, but is it
running? Y ou wanted to send an email, but is it in your outbox?
...
back on course.
Intelligence
Has anyone noticed that LLMs are getting a lot smarter? Y eah…and they’re going to
...
marks going forward
We’re also getting smarter with model training. Y ou can actually watch this happen
...
1. LLMs are nothing more than text completion engines that mimic the text they1.
see during training.
2. Y ou should empathize with the LLM and understand how it thinks.2.
...
1 Terry Pratchett, The  Fifth  Elephant (New Y ork: Doubleday, 1999)
...
flexible and open-ended experiences. Development will change. Y ou will work in
